...
public interface DoorRepository extends JpaRepository<Door, Long>{
    ...
    List<Door> findDay(@Param("beforeNum") int beforeNum, @Param("afterNum") int afterNum, @Param("minus") int minus);
    ...
    List<Door> findWeekOrMonth(@Param("num") int num, @Param("minus") int minus);
    ...
    @Query("SELECT w FROM Door w WHERE EXTRACT(DATE FROM w.time) = EXTRACT(DATE FROM CURRENT_TIMESTAMP) - 1 " +
            ...
            "AND EXTRACT(MINUTE FROM w.time) = :num")
    List<Door> findWeekOrMonthAndChangeDay(@Param("num") int num);
}
